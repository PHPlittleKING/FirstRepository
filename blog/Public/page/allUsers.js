layui.config({
	...
	//加载页面数据
	var usersData = '';
	$.get("/admin.php/Admin/show", function(data){
		console.log(data);
		usersData = data;
	...
	$("body").on("click",".users_del",function(){  //删除
		...
		layer.confirm('确定删除此用户？',{icon:3, title:'提示信息'},function(index){
			...
			$.post("/admin.php/Admin/del",{'id':id},function(result){
				// console.log(result);
				// alert(id);
		...
		function renderDate(data,curr)
		{
			...
			if(currData.length != 0)
			{
				for(var i=0;i<currData.length;i++)
				{
			    	...
			    	+  '<td>'+currData[i].admin_last_ip+'</td>'
			    	+  '<td>'+currData[i].admin_email+'</td>'
			    	+  '<td>'+currData[i].admin_last_time+'</td>'
			    	+  '<td>'
					+    '<a class="layui-btn layui-btn-mini users_edit" data-id="'+data[i].article_id+'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+    '<a class="layui-btn layui-btn-danger layui-btn-mini users_del" data-id="'+data[i].admin_id+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +  '</td>'
			    	+'</tr>';
				}
			}
