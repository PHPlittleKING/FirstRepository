layui.config({
	...
	//创建一个编辑器
 	...
 	form.on("submit(addLinks)",function(data){
 		...
 		addLinks = '{"link_name":"'+ $(".linksName").val() +'",';  //网站名称
 		addLinks += '"link_address":"'+ $(".linksUrl").val() +'",';	 //网站地址
 		// addLinks += '"linksDesc":"'+ $(".linksDesc").text() +'",'; //站点描述
 		// addLinks += '"linksTime":"'+ $(".linksTime").val() +'",'; //发布时间
 		addLinks += '"admin_email":"'+ $(".masterEmail").val() +'"}'; //站长邮箱
 		...
 		// alert(addLinks);
 		// console.log(result);
 		// $.post("/admin.php/Link/add",addLinks,function(result){
 		// 	if(result.status)
 		// 	{
