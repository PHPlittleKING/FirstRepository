layui.config({
	...
	//加载页面数据	
	var linksData = '';
	$.get("/admin.php/Link/show", function(data){
		console.log(data);
		linksData = data;
	...
	$("body").on("click",".links_del",function(){  //删除
		...
		var id = _this.attr('data-id');
			$.post("/admin.php/Link/del",{id:'id'},function(result){
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'+currData[i].link_name+'</td>'
			    	+'<td><a style="color:#1E9FFF;" target="_blank" href="'+currData[i].link_address+'">'+currData[i].link_address+'</a></td>'
			    	+'<td>'+currData[i].admin_email+'</td>'
			    	// +'<td>'+currData[i].link_address+'</td>'
			    	// +'<td>'+currData[i].admin_email+'</td>'
			    	+'<td>'+currData[i].link_time+'</td>'
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-mini links_edit"><i class="iconfont icon-edit"></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-mini links_del" data-id="'+data[i].link_id+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
