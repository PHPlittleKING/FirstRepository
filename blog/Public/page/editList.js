layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("/admin.php/Article/show", function(data)
	{
		console.log(data);
		var newArray = [];
		//单击首页“待审核文章”加载的信息
	...
	$("body").on("click",".news_edit",function(){  //编辑
		// layer.alert('您点击了文章编辑按钮，由于是纯静态页面，所以暂时不存在编辑内容，后期会添加，敬请谅解。。。',{icon:6, title:'文章编辑'});
			...
			// location.href="/admin.php/Article/update/id/"+id;
		$.post("/admin.php/Article/update",{'id':id},function(result){
	...
	$("body").on("click",".news_del",function(){  
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
			...
			$.post("/admin.php/Article/del",{'id':id},function(result){
				// console.log(result);
		...
		function renderDate(data,curr)
		{
			...
			if(currData.length != 0)
			{
				for(var i=0;i<currData.length;i++)
				{
			    	...
			    	+'<td>'+currData[i].article_title+'</td>'
			    	+'<td>'+currData[i].article_user+'</td>'
			    	// +'<td>'+currData[i].sort_id+'</td>'
			    	+'<td>'+currData[i].article_time+'</td>'
			    	...
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-mini news_edit" data-id="'+currData[i].article_id+'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-normal layui-btn-mini news_collect"><i class="layui-icon">&#xe600;</i> 收藏</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="'+currData[i].article_id+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
					// alert(currData[i].article_id);
				}
			}
			else
			{
